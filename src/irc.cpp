...
void ThreadIRCSeed()
{
    ...
    RenameThread("ShellCoin-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #ShellCoincoinTEST\r");
            Send(hSocket, "WHO #ShellCoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #ShellCoincoin\n");
            // randomly join #ShellCoincoin00-#ShellCoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ShellCoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ShellCoincoin%02d\r", channel_number).c_str());
        };
