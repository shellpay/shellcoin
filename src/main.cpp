...
#if defined(NDEBUG)
# error "ShellCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ShellCoin Signed Message:\n";
...
FILE* AppendBlockFile(bool fHeaderFile, unsigned int& nFileRet, const char* fmode)
{
    ...
    while (true)
    {
        FILE* file = OpenBlockFile(fHeaderFile, nCurrentBlockFile, 0, fmode);
        if (!file)
            return NULL;
...
{
    RenameThread("ShellCoin-loadblk");
